...
/// Input: logn(32) | q00_half(hn×2 LE) | q01(n×2 LE) | h0(n/8 bytes) | w1(n×2 LE)
/// Output: n×2 bytes (s0 as i16 LE)
#[no_mangle]
pub unsafe extern "C" fn eth_ntt_rebuild_s0(
    input: *const u8, input_len: usize,
    output_out: *mut *mut u8, output_len_out: *mut usize,
) -> i32 {
    let data = slice::from_raw_parts(input, input_len);
    match crate::fx32_fft::rebuild_s0_precompile(data) {
        Some(out) => { write_output(out, output_out, output_len_out); 0 }
        None => -1,
    }
}
